...
if(isset($_POST['submit'])){
   ...
   $file_tmp_name = $_FILES['thumb']['tmp_name'];
   $file_folder = '../uploaded_files/'.$rename_file;

   if($thumb_size > 2000000){
      $message[] = 'image size is too large!';
   }else{
      $add_playlist = $conn->prepare("INSERT INTO `contents`(id, tutor_id, playlist_id, title, description, file, thumb, status) VALUES(?,?,?,?,?,?,?,?)");
      $add_playlist->execute([$id, $tutor_id, $playlist, $title,$rename_file, $description, $rename_thumb, $status]);
...
<section class="video-form">
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <p>select pdf file <span>*</span></p>
      <input type="file" name="file" accept=" .pdf" required class="box">
